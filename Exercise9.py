...
class particle(object):
    ...
    def state(self,px,py,pz,e,theta,phi,s):
        ...
        self.Pz = pz
        self.E = e
        self.Pt = np.sqrt(np.square(self.Py)+np.square(self.Pz))
        self.Theta = theta
        self.Phi = phi
        self.splitable = s
...
#plot.show()
    ...
    for ipart1 in range(len(parts)):
        ...
        if dij<diB:
            ...
            Pt_temp = parts[ipart1].Pt
            if Pt_temp>parts[PtF1num[irunning]].Pt:
                PtF2num[irunning]=PtF1num[irunning]
                PtF1num[irunning]=ipart1
            elif Pt_temp>parts[PtF2num[irunning]].Pt:
                PtF2num[irunning]=ipart1

    ftheta1 = parts[PtF1num[irunning]].Theta
    ftheta2 = parts[PtF2num[irunning]].Theta
    fphi1 = parts[PtF1num[irunning]].Phi
    fphi2 = parts[PtF2num[irunning]].Phi
    delta = np.sqrt(np.square(ftheta1-ftheta2) + np.square(fphi1-fphi2))
    m=parts[PtF1num[irunning]].E*parts[PtF2num[irunning]].E*delta
    M.append(m)
    
drawhist(count, 10, "Number of reco_jets", "Number of shower", "jet reconstruction")
drawhist(finalNum, 30, "Number of all_jets", "Number of shower", "jet production")
drawhist(M, 30, "Mass of di-jet", "Number of shower", "Mass")
